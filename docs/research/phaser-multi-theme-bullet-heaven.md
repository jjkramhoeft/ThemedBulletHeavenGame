...
14. **`VIDEO_PLAYING` never fires for a muted (`noAudio`) clip's first start.** Found during M1. `playSuccess` attaches the element's DOM listeners only after the `play()` promise resolves ([Video.js `addEventHandlers`, "Set these _after_ calling `video.play`"](https://github.com/phaserjs/phaser/blob/v4.2.1/src/gameobjects/video/Video.js)). By then a muted, autoplaying element has already fired `playing`. For the same reason, a Video created from a cache key has width 0 at creation, so its display origin is wrong until you call `setOrigin` again. The fix used in `Cutscene.ts`: poll `isPlaying() && getCurrentTime() > 0` in `update()`, then set the origin and scale and start the soundtrack there.
